    ...
    describe 'Capybara#disable_animation' do
      context 'when set to `true`' do
        ...
        it 'should scroll the page instantly', requires: [:js] do
          @animation_session.visit('with_animation')
          @animation_session.execute_script('window.scrollTo(0, 500)')
          scrollY = @animation_session.evaluate_script('window.scrollY')
      ...
      context 'when set to `false`' do
        ...
        it 'should scroll the page with a smooth animation', requires: [:js] do
          ...
          @animation_session.execute_script('window.scrollTo(0, 500)')
          scrollY = @animation_session.evaluate_script('window.scrollY')
          # scrollY measured over 0.5 seconds: 0, 75, 282, 478, 500
          expect(scrollY).to be < 500
        end
      end
